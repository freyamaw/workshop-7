...
hard to see relationship with geom_point. geom_jitter slightly randomises points so correlation is easier to see
...
y-intercept = 3.88. When avg beauty = 0, avg score = 3.88
R^2 = 0.035 (very low!!)
...
```{r eval = FALSE}
ggplot(evals,
       aes(x = bty_avg, y = score, colour = gender)
       ) + 
  geom_jitter() +
  geom_smooth(method = lm, se = F)
```
